import numpy as np 
import pandas
from qea import QuantumEvAlgorithm
from ttest_functions import f,g,rastrigin,rosenbrock, griewank, michael, schwefel, dropwave, schaffer_2, equation

up = 5*np.ones(50)
low = -5*np.ones(50)
optimizer = QuantumEvAlgorithm(g, n_dims = 50,upper_bound= up, lower_bound= low,sigma_scaler = 1.04,
                                   mu_scaler = 20, elitist_level = 4, ros_flag= False, saving_interval=500)

results = optimizer.training(N_iterations=2000, sample_size= 15, save= True ,filename='q11.npz')

print(results)


#Todo en orden